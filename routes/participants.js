  ...
  try {
    ...
    if (user) {
      for (let i = 0; i < user.participants.length; i++) {
        const obj = await Participant.findById(user.participants[i]);
        data.push({
          first_name: obj.first_name, last_name: obj.last_name, id: obj.id, obj_id: obj._id,
        });
      }
    }
  } catch (err) {
...
async function delete_participant(user_id, participants_id) {
  try {
    Participant.findByIdAndDelete(participants_id, (err, docs) => {
      if (err) {
        console.log(err);
      } else {
...
router.post('/update', (req, res) => {
  ...
  Participant.findOneAndUpdate({ id }, { first_name, last_name }, { new: true }, (err, doc) => {
    if (err) {
      console.log(err);
  ...
  User.findOne({ _id: user.id }) // first, find current user
    .then((user) => {
      if (user) {
        ...
        Participant.findOne({ id })
          .then((participant) => {
            // if participant already exits
            // search him in user's participant list,
            // if exists in list do nothing, if don't - add to list
...
router.delete('/:id', (req, res) => {
  const promise = delete_participant(req.user, req.params.id);
  promise
    .then((p) => {
