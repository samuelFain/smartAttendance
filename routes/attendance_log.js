		...
		for (let i = 0; i < sessions.length; i++) {
			...
			for (let j = 0; j < sessions[i].participants.length; j++) {
				if (user.participants.includes(sessions[i].participants[j])) {
					...
					if (participant) {
						curr_participants.push({first_name: participant.first_name, last_name: participant.last_name, id: participant.id, obj_id: participant._id});
					}
				}
			}
			data.push({user: user, participants: curr_participants, date: sessions[i].date});
		}
		return data;
	} catch (err) {
